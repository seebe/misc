...
static void rcar_du_crtc_set_display_timing(struct rcar_du_crtc *rcrtc)
{
	...
	if (rcar_du_has(rcdu, RCAR_DU_FEATURE_RZG2L)) {
		...
		param.frequency = 0;
		param.pl5_refdiv = 1;	/* (1-63) values other than 1 are not recommended */
		param.pl5_postdiv1 = 1;	/* (1-7) values other than 1 are not recommended */
		param.pl5_postdiv2 = 1;	/* values other than 1 are not recommended */
		param.pl5_divval = 0;
...
printk("-------pix_clk %u\n",pix_clk);//asdf
		...
		if (pll5_clk > 1500000000) {
			if ((pll5_clk > 3000000000) || parallelOut) {
...
printk("-------divide_val %llu\n",divide_val);//asdf
		param.pl5_intin = divide_val / OSCLK_HZ;
printk("-------pl5_intin %u\n",param.pl5_intin);//asdf

		/* Denominator portion (multiplied by 16k to become an integer) */
		divide_val = divide_val % OSCLK_HZ;	/* remove integer portion */
		divide_val = divide_val * 16*1024*1024;	/* convert from decimal to integer */
		divide_val = divide_val / OSCLK_HZ;	/* now we can divide */
		param.pl5_fracin = divide_val;
printk("-------pl5_fracin %u\n",param.pl5_fracin);//asdf
